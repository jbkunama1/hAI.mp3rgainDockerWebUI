...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;font-family:Inter,Arial,sans-serif;background:linear-gradient(180deg,#020617,#0f172a);color:var(--text)}
    ...
    input[type=text],select{width:100%;padding:12px 14px;border-radius:12px;border:1px solid var(--line);background:var(--panel2);color:var(--text)}
...
<body>
  <div class="wrap">
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <table>
          ...
          <tr><td>2.</td><td>Modus waehlen und Job starten</td></tr>
    ...
    <div class="card" style="margin-top:20px">
      ...
      {% if jobs %}
      <table>
        <thead><tr><th>Zeit</th><th>Job-ID</th><th>Modus</th><th>Dateien</th><th>Download</th></tr></thead>
        <tbody>
          {% for job in jobs %}
          <tr>
            <td>{{ job.created }}</td>
...
def write_meta(job_dir, job_id, mode, count):
...
def index():
    return render_template_string(TEMPLATE, jobs=list_jobs(),
                                  input_dir=INPUT_DIR, output_dir=OUTPUT_DIR, port=APP_PORT)
